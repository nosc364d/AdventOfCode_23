...
function isInRange(toTest: number, start: number, end: number): boolean {
  console.log("testing: [" + start + ", " + toTest + ", " + end + "]");
        ...
        lines.map((line) => {
          if (line.match(/[0-9]/)) {
            ...
            if (mapping[0] !== "seeds:") {
              ...
              if (
                isInRange(
                  seedsAsNums[seedIndex] + thisSeeds,
                  src,
                  src + range
                ) &&
                check
                ...
                // console.log("found in range from " + src + " to " + (src + range));
                // console.log("now " + (dst + (seedsAsNums[seedIndex] - src)));
                seedsAsNums[seedIndex] =
                  dst + (seedsAsNums[seedIndex] + thisSeeds - src);
                check = false;
              }
              if (currentLowestResult > seedsAsNums[seedIndex])
                currentLowestResult = seedsAsNums[seedIndex];
            }
          } else {
            check = true;
          }
